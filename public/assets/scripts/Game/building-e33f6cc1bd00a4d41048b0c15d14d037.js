        ...
        const loadPopupContent = () => {
            fetch(`/api/buildings/${buildingId}/popup-content`)
                ...
                .then(html => {
                    ...
                    if (container) {
                        // Remplacer le conteneur entier
                        const newContainer = document.createElement('div');
                        newContainer.innerHTML = html;
                        const newContent = newContainer.firstChild;
                        container.parentNode.replaceChild(newContent, container);
                        // Initialiser le timer après chargement du contenu
                        if (window.deliveryTimerManager) {
                            window.deliveryTimerManager.initTimer(buildingId);
                        }
                    }
                })
